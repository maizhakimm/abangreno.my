...
const NAV_ITEMS = [
  ...
  { href: "/dashboard/service-areas", label: "Kawasan Servis" },
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8 md:flex-row">
      <aside className="shrink-0 md:w-56">
        <nav className="flex gap-2 overflow-x-auto md:flex-col md:overflow-visible">
          {NAV_ITEMS.map((item) => (
            <Link key={item.href} href={item.href} className="focus-ring whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium hover:bg-white hover:text-brand">
              {item.label}
            </Link>
          ))}
        </nav>
